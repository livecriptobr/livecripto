...
import { ArrowLeft, Copy, Check, Loader2, Zap } from 'lucide-react'
...
export default function LightningPayment({ data, onBack }: Props) {
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
      try {
        const res = await fetch(`/api/public/donate/status?provider=coinsnap&id=${data.donationId}`)
  ...
  const handleCopy = async () => {
    if (data.invoice) {
      await navigator.clipboard.writeText(data.invoice)
      setCopied(true)
  ...
  if (status === 'paid') {
    return (
      <div className="w-full max-w-md bg-zinc-900 rounded-2xl p-8 border border-zinc-800 text-center">
        ...
        <p className="text-zinc-400">Obrigado pela sua doacao via Lightning!</p>
      </div>
    )
  }
    ...
    <div className="w-full max-w-md bg-zinc-900 rounded-2xl p-8 border border-zinc-800">
      ...
      {data.qrCode && (
        <div className="bg-white p-4 rounded-xl mb-6 mx-auto w-fit">
          <img src={data.qrCode} alt="QR Code Lightning" className="w-48 h-48" />
        </div>
      )}

      {data.invoice && (
        <div className="space-y-2">
          <p className="text-sm text-zinc-400">Ou copie a invoice:</p>
      ...
      <div className="flex items-center justify-center gap-2 mt-6 text-zinc-400">
        ...
        <span className="text-sm">Aguardando pagamento Lightning...</span>
      </div>
    </div>
  )
}
